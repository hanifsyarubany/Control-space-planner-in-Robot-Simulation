...
GraphPlanner::GraphPlanner(ros::NodeHandle &nh) : 
    ...
    use_actionlib_(false),  // Initialize all members
    all_goals_completed_(false),
    has_active_goal_(false)
{
    ...
    goal_done_sub_ = nh_.subscribe("/planner/goal_reached", 10,
                                 &GraphPlanner::GoalDoneCb, this);
    LoadCSV(csv_path);
}
...
void GraphPlanner::GoalDoneCb(const std_msgs::Bool::ConstPtr& msg)
{
    if (!msg->data) return;              // ignore false (if you ever send it)
    if (!has_active_goal_) return;       // guard against duplicates
    if (goal_timeout_active_) {          // cancel timer
        goal_timeout_timer_.stop();
        goal_timeout_active_ = false;
    }
    ...
    if (current_goal_index_ >= nodes.size()) {
        ...
        ROS_INFO("[GraphPlanner] Sequence finished.");
        return;
    }

    ROS_INFO("[GraphPlanner] Goal done → sending next waypoint");
    sendNextGoal();  // chain immediately
}


void GraphPlanner::sendNavigationGoal(double x, double y, double yaw) {
    if (goal_timeout_active_) {
        goal_timeout_timer_.stop();
    }
    if(use_actionlib_ && ac_.isServerConnected()) {
        ...
        bool finished = ac_.waitForResult(ros::Duration(100.0));
        if(finished) {
        ...
        // Method 2: Using simple publisher (good fGoalDoneCbor testing)
    ...
    // Start 80-second timeout timer
    goal_timeout_timer_ = nh_.createTimer(
        ros::Duration(80.0),
        &GraphPlanner::GoalTimeoutCallback,
        this,
        true  // oneshot = true (only fires once)
    );
    goal_timeout_active_ = true;
}

void GraphPlanner::GoalTimeoutCallback(const ros::TimerEvent& event) {
    if (!has_active_goal_) return; // Already completed or canceled

    ROS_WARN("[GraphPlanner] Goal timeout after 60 seconds! Proceeding to next waypoint.");
    
    goal_timeout_active_ = false;
    has_active_goal_ = false;

    // Decide policy: skip failed goal and go to next?
    if (current_goal_index_ < nodes.size()) {
        ROS_INFO("[GraphPlanner] Skipping timed-out goal and sending next.");
        sendNextGoal();
    } else {
        all_goals_completed_ = true;
        ROS_INFO("[GraphPlanner] All goals processed (including timeout).");
    }
}
...
int main(int argc, char **argv)
{
    ...
    ROS_INFO("Initiate: GraphPlanner ...");
    GraphPlanner node(_nh);
    node.run();
    ros::spin(); 
    ROS_INFO("Terminate: GraphPlanner ...");
