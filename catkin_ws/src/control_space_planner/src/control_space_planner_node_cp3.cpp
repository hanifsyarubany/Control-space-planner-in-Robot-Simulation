...
{
  // Subscriber
  subOccupancyGrid = nh.subscribe("/map",1, &MotionPlanner::CallbackOccupancyGrid, this);
...
void MotionPlanner::PublishCommand(std::vector<Node> motionMinCost)
{
  ...
  if (bGetGoal && bGetLocalNode) {
    double distToGoal = std::sqrt(
        (this->ego_x - this->goal_x)*(this->ego_x - this->goal_x) +
        (this->ego_y - this->goal_y)*(this->ego_y - this->goal_y)
    );
...
void MotionPlanner::Plan()
{
  // Ensure we have both odometry (global pose estimate) and goal
  if (this->bGetEgoOdom && this->bGetGoal) {
    // Optional: publish debug pose (still useful for visualization in base_link)
    geometry_msgs::PoseStamped localPose = GlobalToLocalCoordinate(this->goalPose, this->egoOdom);
    localPose.header.frame_id = "base_link";
    pubTruncTarget.publish(localPose);
    this->bGetLocalNode = true;

    ROS_INFO_STREAM_THROTTLE(1.0, "[GoalGlobal] (x,y,yaw)=("
      << this->goal_x << "," << this->goal_y << "," << this->goal_yaw << ")");
  }

  // Generate motion primitives in GLOBAL frame
  motionCandidates = GenerateMotionPrimitives(this->localMap); // note: "localMap" is now actually the global /map
...
std::vector<std::vector<Node>> MotionPlanner::GenerateMotionPrimitives(nav_msgs::OccupancyGrid localMap)
{
  ...
  */
  ...
  for (int i = 0; i < num_candidates; i++) {
    ...
    Node startNode(this->ego_x, this->ego_y, 0, this->ego_yaw, angle_delta, cost_control, 0, 0, -1, false);
                                              ...
                                              nav_msgs::OccupancyGrid localMap)
{
  ...
  */
  ...
  while (progress < maxProgress) {
    ...
    if (this->bGetGoal) {
      double distGoal = std::sqrt(
        (currMotionNode.x - this->goal_x) * (currMotionNode.x - this->goal_x) +
        (currMotionNode.y - this->goal_y) * (currMotionNode.y - this->goal_y)
      );
    ...
    // <<MODIFICATION>>
    double dx = currMotionNode.x - this->ego_x;
    double dy = currMotionNode.y - this->ego_y;
    double LOS_DIST = std::sqrt(dx*dx + dy*dy);
    double LOS_YAW = std::atan2(dy, dx);
  ...
  */
  ...
  if (motionPrimitives.size() != 0) {
    ...
    for (auto& motionPrimitive : motionPrimitives) {
      ...
      if (this->bGetGoal) {
          double th_goal = std::atan2(
              this->goal_y - motionPrimitive.back().y,
              this->goal_x - motionPrimitive.back().x
          );
