...
class MotionPlanner
{
  public:
    ...
    double MAX_SENSOR_RANGE = 8.0; // [m] maximum sensor range (realsense)
    double WHEELBASE = 0.26; // [m] wheelbase of the vehicle (our RC vehicle is near 0.26 m)
    ...
    double MAX_PROGRESS = 1.0; // [m] max progress of motion

    double ARRIVAL_THRES = 0.5; // [m] distance threshold for arrival

    // - cost weights
    double W_COST_DIRECTION      =  1.0; // -- offline cost
    double W_COST_TRAVERSABILITY = 100.0; // -- online cost
    
    // - collision checking
    double INFLATION_SIZE = 0.25 / mapResol; // [grid] inflation size [m] / grid_res [m/grid]
    double SAFE_LOOKAHEAD_DIST = 0.1;
    double LOOKAHEAD_DIST = 0.8; // [m] lookahead distance for collision cheking
  ...
  private:
    ...
    std::vector<geometry_msgs::PoseStamped> static_global_path_;
    int current_path_index_ = 0;
    bool path_initialized_ = false;
    bool goal_reached_sent_ = false; 
